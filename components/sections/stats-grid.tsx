      ...
      <div className="container space-y-12">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {trustMetrics.map((metric) => (
            <div key={metric.label} className="rounded-3xl border border-border bg-white p-6 shadow-card">
              <p className="text-3xl font-semibold text-primary">{metric.value}</p>
              <h3 className="mt-3 text-lg font-semibold text-primary">{metric.label}</h3>
        ...
        <div className="rounded-[2rem] border border-border bg-primary px-6 py-5 text-primary-foreground shadow-soft md:flex md:items-center md:justify-between md:px-8">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-white/70">Credibility Matters</p>
            <h3 className="mt-2 text-2xl font-semibold">Recognized guidance for degrees that support real academic progression</h3>
          </div>
